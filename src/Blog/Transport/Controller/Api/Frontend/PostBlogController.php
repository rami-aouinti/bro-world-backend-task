...
#[AsController]
#[OA\Tag(name: 'Blog')]
readonly class PostBlogController
{
    ...
    public function __invoke(SymfonyUser $symfonyUser,Request $request): JsonResponse
    {
        ...
        $blog->setSlug($request->request->get('title'));
        $blog->setBlogSubtitle($request->request->get('subTitle'));
        $blog->setAuthor(Uuid::fromString($symfonyUser->getUserIdentifier()));
        if($request->request->get('logo')) {
            $blog->setLogo(Uuid::fromString($request->request->get('logo')));
        }
        $blog->setColor($request->request->get('color'));
        ...
        $this->cache->delete('blog_user_' . $symfonyUser->getUserIdentifier() . '_public');
